...
col5.metric("Output BTC", f"{total_output_btc:.4f}")
...
st.markdown(
    ...
    <div class="case-card">
        ...
        <p>
            The app detected <strong>{len(repeated_values)} repeated output value groups</strong>.
            For example, the same BTC amount appears many times across different outputs.
        </p>
        <p>
            This matters because repeated standard-sized outputs make it harder to directly match one input
            participant to one output recipient.
        </p>
    </div>
    """,
    unsafe_allow_html=True,
...
st.write(
    "This graph summarises the selected transaction structure. Funds from many inputs are pooled into one shared "
    "transaction, then redistributed across many outputs. Green nodes highlight repeated output amounts, which "
    "can make ownership tracing more difficult."
)
...
st.write(
    "This table is the strongest indicator in this case. Repeated output denominations suggest funds may be "
    "redistributed in standard-sized chunks rather than flowing directly from one sender to one recipient."
)
