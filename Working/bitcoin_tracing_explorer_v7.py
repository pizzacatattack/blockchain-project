        ...
        if amount_received_by_searched > 0:
            ...
            for source in source_addresses:
                received_rows.append({
                    ...
                    "Total Received in Transaction AUD": btc_to_aud(amount_btc, btc_aud_rate)
                })
...
with tab_address:
    ...
    if address:
        try:
            ...
            st.write("These rows show loaded transactions where the searched address received Bitcoin. If a transaction has multiple source addresses, the transaction may appear more than once. The received amount is the total received by the searched address in that transaction.")
