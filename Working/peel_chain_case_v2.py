...
# Fixed case settings
# -----------------------------
CASE_NAME = "Dharma / Xorist Peel-Chain Case Study"
...
with st.sidebar:
    ...
    max_hops = st.slider("Maximum hops", min_value=2, max_value=20, value=DEFAULT_MAX_HOPS)
    graph_steps = st.slider("Graph steps to show", min_value=2, max_value=12, value=DEFAULT_GRAPH_STEPS)
    run_button = st.button("Run peel-chain trace", type="primary", use_container_width=True)
    clear_cache = st.button("Clear cached API data", use_container_width=True)

if clear_cache:
    st.cache_data.clear()
    st.success("Cache cleared. Run the trace again to refetch blockchain data.")

if not run_button:
    st.info("Use the sidebar to run the peel-chain trace.")
    st.stop()
