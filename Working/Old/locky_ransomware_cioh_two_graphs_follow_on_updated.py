...
st.markdown(
    ...
    <p class="small-note">
    The later input addresses were not found as ransomware-labelled addresses in the Ransomwhere dataset, so this should be treated as follow-on tracing only.
    </p>
    </div>
    """,
    unsafe_allow_html=True,
...
with st.expander("Show full input and output address tables"):
    ...
    input_table = build_display_table(
        ...
        "Input Address",
        "Input BTC",
        role_func=lambda x: "Seed address" if x == SEED_ADDRESS else "Other Locky-labelled input",
    )
