...
def render_metric_row(results: Dict[str, Any]) -> None:
    """Render the headline metric cards for the dashboard.

    Args:
        results: Results dictionary returned by run_analysis().
    """
    ...
    col1.metric("Seed BTC", f"{results['seed_btc']:.4f}")
    col2.metric("Main flow BTC", f"{results['main_flow_btc']:.4f}")
    col3.metric("Peel events", len(valid_peels))
    col4.metric("Largest peel", f"{largest_peel:.4f} BTC")

    col5, col6, col7, col8 = st.columns(4)
    col5.metric("Total peeled", f"{total_peeled:.4f} BTC")
    col6.metric("Hops analysed", len(results["hop_results"]))
...
def main() -> None:
    ...
    st.set_page_config(
        page_title="Peel-Chain Case Dashboard",
        page_icon="₿",
        layout="wide",
    )

    st.title("🕵️ Peel-Chain Case Dashboard")
    st.caption(
        "Investigator-style case dashboard for tracing possible peel-chain behaviour "
        "using public Bitcoin transaction data."
    )
    ...
    with st.sidebar:
        ...
        seed_address = st.text_input("Seed address", value=DEFAULT_SEED_ADDRESS)

        st.header("Investigation settings")
        min_btc = st.number_input(
            "Minimum BTC threshold",
            min_value=0.0,
            value=DEFAULT_MIN_BTC,
            step=0.001,
            format="%.6f",
        )
        max_hops = st.slider("Maximum main-flow hops", min_value=1, max_value=30, value=DEFAULT_MAX_HOPS)
        graph_steps = st.slider("Graph steps to display", min_value=2, max_value=15, value=DEFAULT_GRAPH_STEPS)

        st.header("Branch settings")
        branch_top_n = st.slider("Top branches per address", min_value=1, max_value=10, value=DEFAULT_BRANCH_TOP_N)
        branch_max_hops = st.slider("Maximum branch depth", min_value=1, max_value=5, value=DEFAULT_BRANCH_MAX_HOPS)
        branch_max_pages = st.slider(
            "Max API pages per branch address",
            min_value=1,
            max_value=5,
            value=DEFAULT_BRANCH_MAX_PAGES_PER_ADDRESS,
        )

        run_button = st.button("Run case investigation", type="primary", use_container_width=True)
        clear_cache = st.button("Clear cached API data", use_container_width=True)

    if clear_cache:
        st.cache_data.clear()
        st.success("Cache cleared. Run the analysis again to refetch data.")

    st.warning(
        "Case note: this dashboard uses heuristics. It can flag possible peel-chain patterns, "
        "but it does not prove ownership, attribution or criminal activity."
    )

    with st.expander("Case objective", expanded=not run_button):
        st.markdown(
            """
            This dashboard starts from a Bitcoin seed address, fetches confirmed transaction data
            from the Blockstream API, follows the largest onward outputs as a possible
            peel-chain path and estimates how much BTC may have been peeled away at each step.

            The goal is to package the case study like an investigation file: key indicators,
            flow table, branch checks, graph and raw supporting data.
            """
        )

    if not run_button:
        st.info("Use the sidebar settings, then click **Run case investigation**.")
        return

    if not seed_address.strip():
        st.error("Please enter a seed address.")
        return

    with st.spinner("Opening case file, fetching transactions and tracing possible peel-chain behaviour..."):
        try:
            results = run_analysis(
                case_name=case_name,
                seed_address=seed_address.strip(),
                min_btc=min_btc,
                max_hops=max_hops,
                graph_steps=graph_steps,
                branch_max_hops=branch_max_hops,
                branch_top_n=branch_top_n,
                branch_max_pages_per_address=branch_max_pages,
            )
    ...
    overview_tab, flow_tab, branch_tab, raw_tab, notes_tab = st.tabs(
        ["Case summary", "Main flow", "Branch analysis", "Evidence tables", "Investigator notes"]
    )

    with overview_tab:
        st.header("Case summary")
        st.subheader("Entry point and main lead")
        st.write("**Seed address:**", results["seed_address"])
        st.write("**Main forward address:**", results["main_flow_address"])

        st.subheader("Top outputs linked to seed address")
        st.dataframe(
        ...
        st.subheader("Key peel-chain indicators")
        st.dataframe(results["stats_df"], use_container_width=True, hide_index=True)

    with flow_tab:
        st.header("Main flow reconstruction")
        st.write(
            "This view follows the largest onward output at each hop as the main investigative lead "
            "and estimates the BTC peeled away between steps."
        )

        st.pyplot(results["graph_fig"], use_container_width=True)

        st.subheader("Main-flow evidence table")
        st.dataframe(
        ...
        with st.expander("Show investigation trace log"):
            for line in results["trace_logs"]:
                st.write("-", line)

        st.subheader("Largest peel event for closer review")
        selected_peel = results["selected_peel"]
        if selected_peel is None:
            st.write("No valid peel event was detected for deeper analysis.")
        else:
            col1, col2, col3 = st.columns(3)
            col1.metric("Step", int(selected_peel["Step"]))
            col2.metric("BTC before peel", f"{selected_peel['BTC']:.6f}")
            col3.metric("Estimated peeled BTC", f"{selected_peel['Estimated Peeled BTC']:.6f}")
            st.write("**Selected address:**", selected_peel["Address"])

            selected_outputs = results["selected_outputs"]
            if selected_outputs.empty:
                st.write("No meaningful onward outputs found above the threshold.")
            else:
                st.dataframe(
                    selected_outputs[["Output Address", "BTC"]].head(10),
                    use_container_width=True,
                    hide_index=True,
                )

    with branch_tab:
        st.header("Limited branch analysis")
        st.write(
            "This section follows a small number of top outputs to check whether funds continue "
            "linearly, branch out or move toward service-like addresses."
        )

        branch_df = results["branch_df"]
        if branch_df.empty:
            st.write("No branch results found above the threshold.")
        else:
            count_df = branch_df["Type"].value_counts().rename_axis("Type").reset_index(name="Count")
            st.subheader("Branch classification counts")
            st.dataframe(count_df, use_container_width=True, hide_index=True)

            st.subheader("Branch trace evidence table")
            st.dataframe(branch_df, use_container_width=True, hide_index=True)

    with raw_tab:
        st.header("Evidence tables")
        st.write("These tables support the dashboard findings and help check the underlying transaction data.")

        with st.expander("Large individual outputs", expanded=True):
        ...
        with st.expander("Output table"):
            st.dataframe(results["df_outputs"].head(200), use_container_width=True, hide_index=True)

    with notes_tab:
        st.header("Investigator notes")
        st.markdown(
            """
            ### Investigative logic
            - The dashboard treats the largest onward output as the main lead for the continuing flow.
            - The estimated peeled amount is the difference between one main-flow step and the next.
            - Very large or high-activity destinations may indicate service-like behaviour.

            ### Important limitations
            - The dashboard does not prove common ownership of addresses.
            - Exchange batching, wallet behaviour, CoinJoin and other privacy techniques can break simple assumptions.
            - Labels such as **Main flow**, **Peel** and **Service / Exchange** are investigative indicators, not verified attribution.

            ### Why this is still useful
            This dashboard shows how public transaction data can be turned into a structured case view,
            allowing an analyst to identify a likely main flow, estimate peeled amounts and document where the trail becomes less linear.
            """
        )
