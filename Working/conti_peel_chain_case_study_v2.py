...
def draw_consolidation_graph(consolidation: Dict[str, Any]) -> plt.Figure:
    """Draw the transaction where the traced 20 BTC output merges with larger funds."""
    G = nx.DiGraph()
    tx_node = "Consolidation transaction"
    labels = {tx_node: "Consolidation\ntransaction"}
    pos = {tx_node: (0, 0)}

    inputs_df = consolidation["inputs"].sort_values("Input BTC", ascending=False).reset_index(drop=True)
    outputs_df = consolidation["outputs"].sort_values("Output BTC", ascending=False).reset_index(drop=True)

    # Put the traced input first visually, then the other inputs by size.
    traced = inputs_df[inputs_df["Role"] == "Traced peel-chain output"]
    other = inputs_df[inputs_df["Role"] != "Traced peel-chain output"]
    visual_inputs = pd.concat([traced, other], ignore_index=True)

    for i, row in visual_inputs.iterrows():
        node = f"Input {i}"
        G.add_edge(node, tx_node)
        role = "Traced input" if row["Role"] == "Traced peel-chain output" else "Other input"
        labels[node] = f"{role}\n{row['Input BTC']:.4f} BTC"
        pos[node] = (-3.2, 1.6 - (i * 1.05))

    for i, row in outputs_df.iterrows():
        node = f"Output {i}"
        G.add_edge(tx_node, node)
        labels[node] = f"Consolidated output\n{row['Output BTC']:.4f} BTC"
        pos[node] = (3.2, 0.2 - (i * 1.2))
    ...
    for node in G.nodes():
        ...
        elif label.startswith("Traced input"):
            node_colours.append("#f97316")
    ...
    legend_handles = [
        mpatches.Patch(color="#f97316", label="Final traced peel-chain output"),
        mpatches.Patch(color="#93c5fd", label="Other consolidation input"),
        mpatches.Patch(color="#111827", label="Consolidation transaction"),
        mpatches.Patch(color="#22c55e", label="Consolidated output"),
    ]
    ax.legend(handles=legend_handles, loc="best")
    ax.set_title("Consolidation phase: traced output merges into a larger transaction", pad=18)
    ax.axis("off")
    fig.tight_layout()
    return fig
...
st.subheader("2. Consolidation phase")
if consolidation is None:
    ...
    st.pyplot(draw_consolidation_graph(consolidation), use_container_width=True)
...
st.subheader("3. Main-flow table")
display_cols = [
    "Step",
    "Date Created",
    "Continuing Address",
